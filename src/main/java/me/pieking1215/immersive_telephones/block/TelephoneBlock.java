package me.pieking1215.immersive_telephones.block;
...
import me.pieking1215.immersive_telephones.util.Utils;
...
public class TelephoneBlock extends Block {
    ...
    private static final Map<Direction, VoxelShape> SHAPES_BASE = new HashMap<>();
    ...
    TelephoneBlock(Properties properties) {
        ...
        Utils.generateRotatedShapes(SHAPE, SHAPES);
        Utils.generateRotatedShapes(SHAPE_BASE, SHAPES_BASE);
    ...
    public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        return state.get(HANDSET) ? SHAPES.get(state.get(FACING)) : SHAPES_BASE.get(state.get(FACING));
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if(tileEntity instanceof TelephoneTileEntity) {
            ...
            if(handStack.getItem() instanceof DyeItem){
                tel.dyed(DyeColor.getColor(handStack));
                ...
                if(worldIn.isRemote()){
                    ...
                    if(it.isEmpty()) {
                        ...
                        if(player.isSneaking() && it.getItem() instanceof HandsetItem && tel.isTheHandset(it)){
                            // hang up
                            tel.getWorld().playSound(null, tel.getPos().getX() + 0.5, tel.getPos().getY() + 0.5, tel.getPos().getZ() + 0.5, player.isOnGround() ? SoundEvents.UI_BUTTON_CLICK : SoundEvents.BLOCK_ANVIL_LAND, SoundCategory.BLOCKS, 1.0f, 1.0f);
