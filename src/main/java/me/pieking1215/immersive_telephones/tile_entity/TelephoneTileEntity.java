...
public class TelephoneTileEntity extends TileEntity implements IImmersiveConnectable, ITickableTileEntity {
    ...
    private List<TelephoneTileEntity> inCallWith = new ArrayList<>();
    ...
    public void tick() {
        ...
        if(!world.isRemote) {
            ...
            if(!dial.isEmpty() && world.getGameTime() - lastDial > 20 * 2){
                findConnectedPhones().stream().filter(t -> t.getName().equals(dial)).findFirst().ifPresent(other -> {
                    other.beingCalled(this);
                });
                dial = "";
            }
...
//                        //}
//                        //System.out.println("HOLDING " + pl.inventory.getItemStack().toString());
//
